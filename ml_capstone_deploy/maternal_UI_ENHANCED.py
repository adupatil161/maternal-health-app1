# ─────────────────────────────────────────────────────────────────────────────
# maternal_UI_CLEAN.py — Maternal Health Risk Predictor (Clean UI)
# Run with: streamlit run maternal_UI_CLEAN.py
# ─────────────────────────────────────────────────────────────────────────────
...
# Custom CSS for clean styling with background colors
st.markdown("""
    <style>
    /* Page background */
    .main {
...
with st.sidebar:
    ...
    **Model Information:**
    ...
    with col1:
        st.markdown("[GitHub](https://github.com/adupatil161/maternal-health-app)")
    with col2:
        st.markdown("[Docs](https://github.com/adupatil161/maternal-health-app)")
...
with st.expander("How This Works"):
    st.markdown("""
    This app uses a machine learning model to predict pregnancy risk levels.
    ...
    **Risk Levels:**
    - Green - Low Risk: Healthy pregnancy with routine monitoring
    - Yellow - Mid Risk: Some concerns, regular checkups recommended
    - Red - High Risk: Significant concerns, immediate medical attention needed
    """)
...
with summary_col1:
    ...
    <div class="metric-card">
        <p style="margin: 4px 0; font-size: 0.75em; opacity: 0.95;">Age</p>
        <p style="margin: 4px 0; font-size: 1.6em; font-weight: bold;">{age}</p>
        <p style="margin: 4px 0; font-size: 0.7em; opacity: 0.85;">years</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"""
    <div class="metric-card" style="margin-top: 8px;">
        <p style="margin: 4px 0; font-size: 0.75em; opacity: 0.95;">Systolic</p>
        <p style="margin: 4px 0; font-size: 1.6em; font-weight: bold;">{sys_bp}</p>
        <p style="margin: 4px 0; font-size: 0.7em; opacity: 0.85;">mmHg</p>
    </div>
    """, unsafe_allow_html=True)
...
with summary_col2:
    st.markdown(f"""
    <div class="metric-card">
        <p style="margin: 4px 0; font-size: 0.75em; opacity: 0.95;">Diastolic</p>
        <p style="margin: 4px 0; font-size: 1.6em; font-weight: bold;">{dia_bp}</p>
        <p style="margin: 4px 0; font-size: 0.7em; opacity: 0.85;">mmHg</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"""
    <div class="metric-card" style="margin-top: 8px;">
        <p style="margin: 4px 0; font-size: 0.75em; opacity: 0.95;">Blood Sugar</p>
        <p style="margin: 4px 0; font-size: 1.6em; font-weight: bold;">{bs:.1f}</p>
        <p style="margin: 4px 0; font-size: 0.7em; opacity: 0.85;">mg/dL</p>
    </div>
    """, unsafe_allow_html=True)
...
with summary_col3:
    st.markdown(f"""
    <div class="metric-card">
        <p style="margin: 4px 0; font-size: 0.75em; opacity: 0.95;">Temperature</p>
        <p style="margin: 4px 0; font-size: 1.6em; font-weight: bold;">{temp:.1f}</p>
        <p style="margin: 4px 0; font-size: 0.7em; opacity: 0.85;">F</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"""
    <div class="metric-card" style="margin-top: 8px;">
        <p style="margin: 4px 0; font-size: 0.75em; opacity: 0.95;">Heart Rate</p>
        <p style="margin: 4px 0; font-size: 1.6em; font-weight: bold;">{heart_rate}</p>
        <p style="margin: 4px 0; font-size: 0.7em; opacity: 0.85;">bpm</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for col_widget, cls, prob in zip([m1, m2, m3], encoder.classes_, probabilities):
        with col_widget:
            ...
            <div style="background: linear-gradient(135deg, {color_start} 0%, {color_end} 100%);
                ...
                <p style="margin: 4px 0; font-size: 1.8em; font-weight: bold;">{cls.title()}</p>
                <p style="margin: 6px 0; font-size: 1.8em; font-weight: bold;">{prob*100:.1f}%</p>
            </div>
            """, unsafe_allow_html=True)
...
st.markdown("---")
st.markdown("""
<div style="text-align: center; padding: 12px; margin-top: 15px; font-size: 0.85em;">
    <small style="color: #667eea; font-weight: 600;">Maternal Health Risk Predictor - Gradient Boosting + SMOTE - Built with Streamlit</small>
</div>
""", unsafe_allow_html=True)
